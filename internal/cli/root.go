...
func newRoot(version string, build buildFunc) *cobra.Command {
	...
	root.AddCommand(newServeCommand(build), newBridgeCommand(build), newModelsCommand(build), newSessionsCommand(build), newSkillsCommand(), newMCPCommand(), newPluginCommand(), newConfigCmd(), newPromptCmd(build), newInstallCmd())
	return root
}
