...
// harness loads the plugin over a two-server mcp.toml: the built echo example and a command
// that does not exist, so one server is ready and one fails in the same load.
type harness struct {
	t       *testing.T
	reg     *plugin.Registry
	notices func() []string
...
func load(t *testing.T) *harness {
	...
	dir := t.TempDir()
	userPath := filepath.Join(dir, "mcp.toml")
	if err := WriteFile(userPath, File{Servers: map[string]ServerConfig{
		...
		"broken": {Transport: TransportStdio, Command: filepath.Join(dir, "nonexistent")},
	}}); err != nil {
		t.Fatal(err)
	}
	var mu sync.Mutex
