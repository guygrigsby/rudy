...
func sessionNotes(ms []rawMsg) []rawMsg {
	...
	for _, m := range ms {
		...
		case protocol.NotifyEntryAppended, protocol.NotifyTurnState,
			protocol.NotifyPermissionRequested, protocol.NotifyStreamDelta:
			out = append(out, m)
		}
	}
	return out
...
// entry, then the current turn.state, then the standing question, then the resume response.
func TestAnAskerAttachingMidQuestionHearsIt(t *testing.T) {
	...
	standing := rawParams[protocol.PermissionRequested](t, before[4])
