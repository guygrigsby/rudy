...
| ShutdownCoordinator | after an accepted same-user `server.shutdown`, flush the success response, transition Server once, stop listener admission, cancel other connection work, close sessions and plugins inside the shutdown budget, close the listener, then complete Server shutdown so the held control connection reaches EOF | Server, Session, Turn, Plugin, connection registry |
...
|---|---|---|---|
| Server.RequestShutdown, CompleteShutdown | `server.shutdown`; response precedes shutdown and connection EOF follows cleanup | `ServerShutdownRequested`, `ServerStopped` | none; Server identity and lifecycle are process facts, not durable records |
