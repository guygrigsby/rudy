...
| `command.run` | TUI, headless, ACP | per class | any attached | `{session_id, name, args: string}` | `{turn_id, notice, session_id}`; `turn_id` set for a command that submitted a prompt, `notice` for one that only reports, `session_id` set when the command opened another session, a fork | `not_found` unknown command | not idempotent; the command decides | runs the plugin.Command's `Run`, then maps its `Action` (`SubmitPrompt`, `Notice`, `Compact`, `SetModel`, `Fork`, `NoAction`) onto the matching domain call |
