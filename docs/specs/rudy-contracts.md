...
|---|---|---|
| Gate | given a `tool_use`, the tool's safety class, the session's mode, session allowances and whether an asker is attached, decide allow, deny or ask; classify the input into a matcher; hold the dangerous set for permissive mode; coalesce a call whose matcher and scope match an ask already outstanding onto that ask, so concurrent calls to one tool put one question to the operator and a session-scope allow binds all of them (ADR 0028) | Turn, Plugin (tool safety), Session (mode, allowances), connection registry (askers) |
...
| concurrent calls matching one outstanding ask put one question | Gate coalesces on matcher and scope | one `permission_decision` per `tool_use`, so each coalesced call still records its own decision row citing the answer that bound it |
