...
|---|---|---|
| Gate | given a `tool_use`, the tool's safety class, the session's mode, session allowances and whether an asker is attached, decide allow, deny or ask; classify the input into a matcher; hold the dangerous set for permissive mode. The rule that concurrent calls matching one outstanding question ask once and share its answer belongs to this service but is enforced at the asker, not in `Evaluate`: the verdict is a pure function of its input and holds no session, while the asker already keys questions by `tool_use` id and holds the session lock (ADR 0028) | Turn, Plugin (tool safety), Session (mode, allowances), connection registry (askers) |
...
| concurrent calls matching one outstanding ask put one question | the asker coalesces on matcher and scope; `Gate.Evaluate` stays pure | one `permission_decision` per `tool_use`, so each coalesced call still records its own decision row citing the answer that bound it |
